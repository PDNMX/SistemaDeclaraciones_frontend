...
class DatosDependiente {
  ...
  async fill(
    ...
    parentescoRelacion = 'BISNIETO(A)',
    rfc = 'CUPU800825569',
    curp = 'FEPM890204HASRRN08',
    ubicacion = 'extranjero',
    actividadLaboral = 'ninguno'
  ) {
    await t.click(Selector('button > span').withText('AGREGAR DEPENDIENTE ECONÓMICO'));
    ...
    await selectField.fill('[formcontrolname="parentescoRelacion"]', parentescoRelacion);
    await textField.fill('[formcontrolname="rfc"]', rfc, replace);
    await radioButtonInput.select('[formControlName="extranjero"]', 'Sí');
  ...
  async save() {
    await t.click(Selector('button > span').withText('GUARDAR CAMBIOS')).wait(5000);
  }
}
